...
export async function openOnSaleforce(conn: Connection, pathParsed: string): Promise<void> {
  const extension = getExtension(pathParsed);
...
class ApprovalProcess extends Metadata {
  async getUrl() {
    const data = await findData(this.conn, this.constructor.name, {
      DeveloperName: this.metadataApiName.split('.')[1],
    });
...
class Field extends Metadata {
  async getUrl() {
    ...
    if (isStandardField(this.metadataApiName)) {
      // Per i campi e gli oggetti standard posso utilizzare come Id il developerName
      ...
      const fieldData = await findData(this.conn, this.constructor.name, {
        DeveloperName: this.metadataApiName,
        TableEnumOrId: objectId,
      });
